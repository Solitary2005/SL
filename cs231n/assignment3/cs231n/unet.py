...
class Unet(nn.Module):
    ...
    def cfg_forward(self, x, time, model_kwargs={}):
        ...
        ##################################################################
        pred_noise_w_condition = self.forward(x, time, model_kwargs)
        model_kwargs["text_emb"] = None
        # pred_noise_wo_condition = self.forward(x, time, {"text_emb":None})
        pred_noise_wo_condition = self.forward(x, time, model_kwargs)
        x = (cfg_scale + 1) * pred_noise_w_condition - cfg_scale * pred_noise_wo_condition
        ##################################################################
    ...
    def forward(self, x, time, model_kwargs={}):
        ...
        for i, down_block in enumerate(self.downs):
            ...
            # print(f"Down block0 {i}: x shape: {x.shape}")
            skip[f"block{i}_resblock1"] = x
            # print(f"block{i}_resblock1")
            x = down_block[1](x, context=context)
            # print(f"Down block1 {i}: x shape after resnet1: {x.shape}")
            skip[f"block{i}_resblock2"] = x
            # print(f"block{i}_resblock2")
            x = down_block[2](x)
            # print(f"Down block1 {i}: x shape after resnet2: {x.shape}")
            

        x = self.mid_block1(x, context=context)
        x = self.mid_block2(x, context=context)
        num_blocks = len(self.downs) - 1

        for i, up_block in enumerate(self.ups):
            x = up_block[0](x)
            # print(f"block0 {i}: x shape: {x.shape}")
            x = torch.cat([x, skip[f"block{num_blocks-i}_resblock2"]], dim=1)
            # print(f"Up block1 {i}: x shape before resnet1: {x.shape}")
            x = up_block[1](x, context=context)
            # print(f"Up block1 {i}: x shape after resnet1: {x.shape}")
            x = torch.cat([x, skip[f"block{num_blocks-i}_resblock1"]], dim=1)
            # print(f"Up block2 {i}: x shape before resnet1: {x.shape}")
            # print(f"block{num_blocks-i}_resblock1")
            x = up_block[2](x, context=context)
            # print(f"Up block2 {i}: x shape after resnet1: {x.shape}")
        ##################################################################
