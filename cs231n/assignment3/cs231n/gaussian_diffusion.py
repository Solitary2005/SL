...
class GaussianDiffusion(nn.Module):
    def __init__(
        ...
        *,
        ...
        #############################################################################
        # alpha_bar_{t-1}
        # 若 alphas_cumprod = [0.9, 0.8, 0.7]，则 alphas_cumprod_prev = [1.0, 0.9, 0.8]
        ...
        # 使用snr做为权重，让噪声较小（信号较强）的步对总损失贡献更大
    ...
    def predict_start_from_noise(self, x_t, t, noise):
        ...
        x_start = (x_t - extract(self.sqrt_one_minus_alphas_cumprod, t, x_t.shape)*noise) / extract(self.sqrt_alphas_cumprod, t, x_t.shape)
    ...
    def predict_noise_from_start(self, x_t, t, x_start):
        ...
        # Look at the coeffs in `__init__` method and use the `extract` function.
        ####################################################################
        pred_noise = (x_t - extract(self.sqrt_alphas_cumprod, t, x_t.shape)*x_start) / extract(self.sqrt_one_minus_alphas_cumprod, t, x_t.shape)
        ####################################################################
        return pred_noise
    ...
    def p_sample(self, x_t, t: int, model_kwargs={}):
        ...
        if self.objective == "pred_noise":
            pred_noise = self.model(x_t, t, model_kwargs)
            pred_x_start = self.predict_start_from_noise(x_t, t, pred_noise)
        elif self.objective == "pred_x_start":
            pred_x_start = self.model(x_t, t, model_kwargs)
            pred_noise = self.predict_noise_from_start(x_t, t, pred_x_start)
        else:
            raise ValueError("objective must be either pred_noise (predict noise) or pred_x_start (predict image start)")
        pred_x_start = torch.clamp(pred_x_start, -1, 1)
        mean, std = self.q_posterior(pred_x_start, x_t, t)
