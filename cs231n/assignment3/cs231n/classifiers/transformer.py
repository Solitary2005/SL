...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        features = self.visual_projection(features).unsqueeze(1) #(N,1,W)
        # attention mask应该是一个(T,T)的，T是解码器目标序列的时间维度，然后是三角阵，下三角(包括对角线)都是true(1)
        T = captions.shape[1]
        mask = torch.tril(torch.ones(T,T,device=captions.device,dtype=captions.dtype)) #上三角置0
        # print(mask)
        output = self.transformer(tgt=captions, memory=features, tgt_mask=mask)
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.transformer(x) #(N, num_patches, emb_dim)
