...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        for i in range(max_len):
            pe[0, i] = torch.tensor([math.sin(i*10000**(-j/embed_dim)) if j%2 == 0 else math.cos(i*10000**(-(j-1)/embed_dim)) for j in range(embed_dim)])
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
    ...
    def forward(self, x):
        ...
        ############################################################################
        output = x + self.pe[:, :S, :].to(x.device).type_as(x)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        query_multihead = query.reshape(N, S, self.n_head, self.head_dim).transpose(1,2) #(N,H,S,E/H)
        key_mutihead = key.reshape(N, T, self.n_head, self.head_dim).transpose(1,2)#(N,H,T,E/H)
        value_multihead = value.reshape(N, T, self.n_head, self.head_dim).transpose(1,2)#(N,H,T,E/H)
        # dk = torch.tensor(value_multihead.shape[-1], dtype=torch.float32)
        # print(query_multihead.shape, key_mutihead.transpose(-1, -2).shape, (N, self.n_head, S, self.head_dim))
        dot = torch.matmul(query_multihead, key_mutihead.transpose(-1, -2)) # (N,H,S,E/H)X(N,H,E/H,T)->(N,H,S,T)
        dot_scaled = dot / math.sqrt(self.head_dim) #(N,H,S,T)
        
        
        if attn_mask is not None:
            attn_mask = attn_mask.unsqueeze(0).unsqueeze(0)
            attn_mask = (attn_mask == 0)
            attn_with_mask = torch.masked_fill(dot_scaled, attn_mask, -1e9) #(N,H,S,T)
            # print(attn_with_mask)
        else:
            attn_with_mask = dot_scaled
        attn_weights = attn_with_mask.softmax(dim=-1)
        attn_weights = self.attn_drop(attn_weights)
        # print(attn_weights.shape, value_multihead.shape)
        output = torch.matmul(attn_weights, value_multihead).transpose(1,2).contiguous()#(N,S,H,E/H) 
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, tgt, memory, tgt_mask=None):
        ...
        ############################################################################
        shortcut = tgt
        #注意是decoder的序列作为Q，来自encoder的序列作为K和V
...
class PatchEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        ############################################################################
        num_patches_W = num_patches_H = int(self.img_size / self.patch_size)
        x = x.reshape(N, C, H, num_patches_W, self.patch_size).reshape(N, C, num_patches_H, self.patch_size, num_patches_W, self.patch_size).transpose(3,4).reshape(N, C, -1, self.patch_size, self.patch_size).transpose(1,2).reshape(N, self.num_patches, self.patch_dim)
        out = self.proj(x) #(N, num_patches, emb_dim)
